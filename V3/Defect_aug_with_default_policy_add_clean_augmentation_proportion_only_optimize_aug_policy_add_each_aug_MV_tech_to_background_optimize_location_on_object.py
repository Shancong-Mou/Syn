...
import segmentation_models_pytorch as smp
model_saving_path = './Defect_aug_with_default_policy_add_clean_only_parameter_MVTech_start30_capsule/_rnd_seed_'+str(rnd_seed)+'_batch_size_2_num_epochs_150_lr_0.00025_lr_schedule_step_size_20model/model30'
...
for ep in range(epochs):
    ...
    ###### calculate total loss
        if optimize_weights:
            tr_loss = weight_image_level_aug*tr_loss_img_level + lam_defect_cut_paste*tr_loss_defect 
        else:
            tr_loss = weight_image_level_aug*tr_loss_img_level + weight_defect_cut_paste*tr_loss_defect_all 
        ...
        if save_all_images:
            ...
            if ep%30==0:
                for num_image in range(numimage):
                    ...
                    axs[4].imshow(K.tensor_to_image(sample_batched_img_level['defect_location_mask'][num_image]))
                    axs[4].set_title('defect_location_mask')
...
with open(store_root+'quant_resut/'+'_rnd_seed_'+str(rnd_seed)+'_btsz_'+str(batch_size)+'_epcs_'+str(num_epochs)+'_lr_'+str(lr)+'_l1_'+ str(weight_image_level_aug)+'_l2_'+ str(lam_defect_cut_paste.item())+'.json', 'w') as f:
    json.dump(res, f)
