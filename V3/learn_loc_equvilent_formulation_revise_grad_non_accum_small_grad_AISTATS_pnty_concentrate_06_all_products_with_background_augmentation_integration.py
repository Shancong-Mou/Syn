...
parser.add_argument('--dice_weight', type=float, required=True)# loss metric: dice or bce
...
parser.add_argument('--warm_up', type=int, required=True) # if use pretrained model, what is the warm_up steps
...
parser.add_argument(
  "--aug_paras_float",  # name on the CLI - drop the `--` for positional/required parameters
  nargs="*",  # 0 or more values expected => creates a list
  type=float,
  default=[0.1, 1.9, 0.1, 1.9, 0.1, 1.9, 0.0, 0.5, 30.0, 0.3, 2.0],  # default if nothing is provided
) #aug_paras_float

parser.add_argument('--lam_weights', 
  nargs="*",  # 0 or more values expected => creates a list
  type=float,
  default=[1, 0.0],  # default if nothing is provided
) # the weight for 2 loss terms, they are: 1. image_level_aug, 2. defect_cut_paste
...
parser.add_argument('--mask_pnty', type=float, default = 0.0)# 
parser.add_argument('--obj_loc', default = False, action='store_true')# 
parser.add_argument('--rnd_loc', default = False, action='store_true')# 
parser.add_argument('--learn_loc', default = False, action='store_true')#   
parser.add_argument('--whole_image_aug', default = False, action='store_true')# r
...
weight_image_level_aug, weight_defect_cut_paste = args.lam_weights
...
mask_pnty = args.mask_pnty 
obj_loc = args.obj_loc
rnd_loc = args.rnd_loc
learn_loc = args.learn_loc
whole_image_aug = args.whole_image_aug
...
if learn_loc:
    image_root = store_root+'learn_location_aug_MVTech_start30_mask_pnty_'+str(mask_pnty)+'/_rnd_seed_'+str(rnd_seed)+'_batch_size_'+str(batch_size)+'_num_epochs_'+str(num_epochs)+'_lr_'+str(lr)+ '_lr_schedule_step_size_'+str(lr_schedule_step_size) + 'loc_map_equvlent_non_accum_small_grad_AISTATS_l1_1e-4_concentration_06_no_penalty_learn_loc/'
    os.makedirs(image_root, exist_ok= True)
    model_root = store_root+'learn_location_aug_MVTech_start30_mask_pnty_'+str(mask_pnty)+'/_rnd_seed_'+str(rnd_seed)+'_batch_size_'+str(batch_size)+'_num_epochs_'+str(num_epochs)+'_lr_'+str(lr)+ '_lr_schedule_step_size_'+str(lr_schedule_step_size) + 'model_no_penalty_learn_loc/'
    os.makedirs(model_root, exist_ok= True)

if rnd_loc:
    image_root = store_root+'rnd_location_aug_MVTech_start30'+'/_rnd_seed_'+str(rnd_seed)+'_batch_size_'+str(batch_size)+'_num_epochs_'+str(num_epochs)+'_lr_'+str(lr)+ '_lr_schedule_step_size_'+str(lr_schedule_step_size) + 'loc_map_equvlent_non_accum_small_grad_AISTATS_l1_1e-4_concentration_06_no_penalty_rnd_loc/'
    os.makedirs(image_root, exist_ok= True)
    model_root = store_root+'rnd_location_aug_MVTech_start30'+'/_rnd_seed_'+str(rnd_seed)+'_batch_size_'+str(batch_size)+'_num_epochs_'+str(num_epochs)+'_lr_'+str(lr)+ '_lr_schedule_step_size_'+str(lr_schedule_step_size) + 'model_no_penalty_rnd_loc/'
    os.makedirs(model_root, exist_ok= True)

if obj_loc:
    image_root = store_root+'obj_location_aug_MVTech_start30'+'/_rnd_seed_'+str(rnd_seed)+'_batch_size_'+str(batch_size)+'_num_epochs_'+str(num_epochs)+'_lr_'+str(lr)+ '_lr_schedule_step_size_'+str(lr_schedule_step_size) + 'loc_map_equvlent_non_accum_small_grad_AISTATS_l1_1e-4_concentration_06_no_penalty_obj_loc/'
    os.makedirs(image_root, exist_ok= True)
    model_root = store_root+'obj_location_aug_MVTech_start30'+'/_rnd_seed_'+str(rnd_seed)+'_batch_size_'+str(batch_size)+'_num_epochs_'+str(num_epochs)+'_lr_'+str(lr)+ '_lr_schedule_step_size_'+str(lr_schedule_step_size) + 'model_no_penalty_obj_loc/'
    os.makedirs(model_root, exist_ok= True)

if whole_image_aug:
    image_root = store_root+'whole_image_aug_MVTech/'+'_rnd_seed_'+str(rnd_seed)+'_batch_size_'+str(batch_size)+'_num_epochs_'+str(num_epochs)+'_lr_'+str(lr)+ '_lr_schedule_step_size_'+str(lr_schedule_step_size) + '/'
    os.makedirs(image_root, exist_ok= True)
    model_root = store_root+'whole_image_aug_MVTech/'+'_rnd_seed_'+str(rnd_seed)+'_batch_size_'+str(batch_size)+'_num_epochs_'+str(num_epochs)+'_lr_'+str(lr)+ '_lr_schedule_step_size_'+str(lr_schedule_step_size) + 'model/'
    os.makedirs(model_root, exist_ok= True)
...
import segmentation_models_pytorch as smp
model_saving_path = root+'whole_image_aug_MVTech/_rnd_seed_'+str(rnd_seed)+'_batch_size_2_num_epochs_150_lr_0.00025_lr_schedule_step_size_20model/model30'
...
# this is for the raw training iamge augmentation
...
for ep in range(epochs):
    ...
    for i_batch, sample_batched_img_level in enumerate(tqdm(dataloader_bkg, desc = 'epoch' + str(ep) +'_train')): # use image_level_aug as main iteration
        ...
        if sample_batched_defect == None:
            if whole_image_aug: # whole image augmentation only iterate through the training data set (defect images)
                break
            iterator = iter(dataloader_defect)
            sample_batched_defect = next(iterator, None)

        # augment background
        sample_batched_img_level_aug, sample_batched_def_loc_mask_aug = image_level_aug_policy(sample_batched_img_level['image'].clone(), sample_batched_img_level['defect_location_mask'].clone()) 
        ###### calculate loss for augmented images tr_loss_defect

        ## object_location_baseline
        if obj_loc:
            ...
            for defects_round in range(number_of_defect_per_image):
                # print(defects_round)
                if defects_round == 0:
                    input = [sample_batched_img_level_aug.clone(), sample_batched_img_level['img_mask'].clone(), defect_batches[defects_round], defect_batches_masks[defects_round], sample_batched_def_loc_mask_aug.clone() ]
                else:
                    input = [output[0], output[1], defect_batches[defects_round], defect_batches_masks[defects_round], sample_batched_def_loc_mask_aug.clone() ]
        ...
        if rnd_loc:
            ...
            for defects_round in range(number_of_defect_per_image):
                # print(defects_round)
                if defects_round == 0:
                    input = [sample_batched_img_level_aug.clone(), sample_batched_img_level['img_mask'].clone(), defect_batches[defects_round], defect_batches_masks[defects_round], torch.ones_like(sample_batched_def_loc_mask_aug.clone()) ]
                else:
                    input = [output[0], output[1], defect_batches[defects_round], defect_batches_masks[defects_round], torch.ones_like(sample_batched_def_loc_mask_aug.clone()) ]
                # with torch.no_grad(): if not learning the policy, then we can keep this with no grad
                output = policy(input) # output is [ Tensor: augmented image, Tensor: Defect mask]
                augmented_image =  output[0].to(device)
                defect_mask = output[1][:,:1,:,:].to(device) # the output mask is C x H x W
        ...
        if learn_loc:
            ...
            defect_batches = torch.split(sample_batched_defect['defect_image'].clone(), batch_size)
            defect_batches_masks = torch.split(sample_batched_defect['defect_image_mask'].clone(), batch_size)
            location_maps = map_pred_net(sample_batched_img_level_aug.to(device)) # batch size == 2
            for defects_round in range(number_of_defect_per_image):
                # print(defects_round)
                if defects_round == 0:
                    input = [sample_batched_img_level_aug.clone(), sample_batched_img_level['img_mask'].clone(), defect_batches[defects_round], defect_batches_masks[defects_round], location_maps.detach().clone().cpu() ]
                else:
        ...
        # image _level_aug_baseline
        if whole_image_aug:
            tr_loss_defect = 0
        ###### calculate loss for image level augmented images tr_loss_img_level
        augmented_img_level_image, augmented_img_level_image_mask = image_level_aug_policy(sample_batched_defect['defect_image'].clone(), sample_batched_defect['defect_image_mask']) 
        ...
        tr_loss_img_level = 0
        for index in range(pred_mask_img_level.shape[0]):
            tr_loss_img_level += ((1-dice_weight)*bce(pred_mask_img_level[index], gt_defect_mask_img_level[index]) +  (dice_weight)*dice(pred_mask_img_level[index], gt_defect_mask_img_level[index]))# input mask is 3 channels, convert to 
            ...
            if i_batch%1 == 0:
                ...
                for hyper_step in range(num_hyper_grad_accumulate):
                    ...
                    for j_batch, sample_val in enumerate(dataloader_val):
                        ...
                        val_loss_ = (1-dice_weight)*bce(pred_mask, gt_defect_mask) +  dice_weight*dice(pred_mask, gt_defect_mask) + mask_pnty*nn.L1Loss()(location_maps,torch.zeros_like(location_maps))
    ...
    if ep%10 == 0:
        ...
        if learn_loc:
            torch.save(map_pred_net, model_root+'map_pred_net'+str(ep))
